...
const PostDetail = () => {
  ...
  // const [newReply, setNewReply] = useState(''); // 답글 관련 상태 제거
  // const [replyingTo, setReplyingTo] = useState(null); // 답글 관련 상태 제거
  ...
  useEffect(() => {
    const fetchPostDetail = async () => {
      try {
        ...
        setError('게시글을 로드하는 데 실패했습니다.'); // 사용자에게 보여줄 에러 메시지 설정
        setLoading(false);
      }
    };
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      const newCommentObj = {
        id: response.data.id,
        content: newComment,
        author: { nickname: userInfo?.nickname || '현재 사용자' }, // 사용자 정보가 있다면 사용, 객체 형태로
